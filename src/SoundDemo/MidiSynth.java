...
import com.java.test.SimpleCodeTester;

import com.java.test.Baseball;

import java.util.StringTokenizer;

import java.util.Scanner;

import com.java.composition.NoteConstants;

import com.java.composition.CompositionFactory;

import com.java.composition.ChordPlayer;

import com.java.composition.CompositionUtil;

import java.util.ArrayList;
import java.util.List;

import com.java.composition.ChordInfo;

import com.java.composition.GenerateChord;
 ...
 */
public class MidiSynth extends JPanel implements ControlContext, MidiSynthInterface{
    ...
    JCheckBox mouseOverCB = new JCheckBox("mouseOver", false);
    ...
    ///////// about composition /////////
    private static final String MAKE_BUTTON_TEXT = "Make";
    private static final String TEST_BUTTON_TEXT = "Test";
    private static final String MELODY_CHECK_TEXT = "Melody";
    private static final String CHORD_CHECK_TEXT = "Chord";
    
    private JCheckBox mMelodyCheck, mChordCheck;
    private JComboBox mKeyCombo, mTempoCombo;
    
    private boolean bTestFlag, bInit; 
...
//            return;
    ...
    class Piano extends JPanel implements MouseListener {
        ...
        public Piano() {
            ...
            System.out.println("blackKeys.size()="+blackKeys.size()+" whiteKeys.size()="+whiteKeys.size());
...
//            playPiano();
        ...
        private void playPiano(){
            key1 = (Key) keys.get(43);
//            key1 = (Key) keys.get(9);     // C7
            key2 = (Key) keys.get(46);
            key3 = (Key) keys.get(48);
...
//            stopPiano();
    ...
    public class Controls extends JPanel implements ActionListener, ChangeListener, ItemListener {
        ...
        private void createCompositionUI(JPanel p){
            JButton makeButton = new JButton(MAKE_BUTTON_TEXT);  
            makeButton.addActionListener(this);               
            
            mMelodyCheck = new JCheckBox(MELODY_CHECK_TEXT);
            mMelodyCheck.addItemListener(this);
            mMelodyCheck.setSelected(true);
            
            mChordCheck = new JCheckBox(CHORD_CHECK_TEXT);
            mChordCheck.addItemListener(this);
            mChordCheck.setSelected(true);
            
            mKeyCombo = new JComboBox();            
                        
            mKeyCombo.addItem('C');
            mKeyCombo.addItem("Db");
            mKeyCombo.addItem('D');
            mKeyCombo.addItem("Eb");
            mKeyCombo.addItem('E');
            
            mKeyCombo.addItem('F');
            mKeyCombo.addItem("Gb");
            mKeyCombo.addItem('G');
            mKeyCombo.addItem("Ab");
            mKeyCombo.addItem('A');
            mKeyCombo.addItem("Bb");
            mKeyCombo.addItem('B');            
            mKeyCombo.setToolTipText("Start Chord");
            mKeyCombo.addItemListener(this);
            
            mTempoCombo = new JComboBox();
            mTempoCombo.addItem(60);
            mTempoCombo.addItem(90);
            mTempoCombo.addItem(120);
            mTempoCombo.setToolTipText("Tempo");
            mTempoCombo.addItemListener(this);
            
            JButton testButton = new JButton(TEST_BUTTON_TEXT);  
            ...
            panel.add(mMelodyCheck);
            panel.add(mChordCheck);
            panel.add(mKeyCombo);
            panel.add(mTempoCombo);
            panel.add(testButton);
            panel.add(Box.createHorizontalStrut(20));
            panel.setBorder(javax.swing.BorderFactory.createTitledBorder("Composition"));
//            panel.getAccessibleContext().setAccessibleName(" E-mail2");
            p.add(panel);
        ...
        public void itemStateChanged(ItemEvent e) {
            ...
            if(e.getStateChange() == e.SELECTED && e.getSource() == mTempoCombo){                
                System.out.println("mTempoCombo.getSelectedIndex()="+mTempoCombo.getSelectedIndex());
                
                GenerateChord.setTempo((Integer) mTempoCombo.getSelectedItem());
                
                
            } else if(e.getStateChange() == e.SELECTED && e.getSource() == mKeyCombo){                
                                
                NoteConstants.setChordList(mKeyCombo.getSelectedIndex());                
                ...
                if(name.equals(MELODY_CHECK_TEXT)){
                    System.out.println("MELODY_CHECK_TEXT");
                    
                } else if(name.equals(CHORD_CHECK_TEXT)){
                    System.out.println("CHORD_CHECK_TEXT");
        ...
        private void calulateStock(){
            
            Scanner scan = new Scanner(System.in);      // 문자 입력을 인자로 Scanner 생성
            
            System.out.println("투자액을 입력하세요:");        
            String inputMoneyStr = scan.nextLine();            // 키보드 문자 입력        
            System.out.println("투자액: \""+ inputMoneyStr + "\"");
            
            System.out.println("투자일수를 입력하세요:");        
            String dayStr = scan.nextLine();            // 키보드 문자 입력        
            System.out.println("투자일수: \""+ dayStr+ "\"");
            
            System.out.println("변동폭을 입력하세요:");        
            String ratioStr = scan.nextLine();            // 키보드 문자 입력        
            System.out.println("변동폭: \""+ ratioStr + "\"");
            
            
            double inputMoney = Double.parseDouble(inputMoneyStr);
            int inputTimes = Integer.parseInt(dayStr);
            StringTokenizer tokenizer = new StringTokenizer(ratioStr, " ");
            double[] ratioArray = new double[inputTimes];
            
            if(inputMoney < 100 || inputMoney > 10000){
                System.out.println("out of range for input money!!!!");
                return;
            }
            
            if(inputTimes < 1 || inputTimes > 10){
                System.out.println("out of range for input times!!!!");
                return;
            }
            ...
            for(int i=0; i<inputTimes; i++){
                ratioArray[i] =  Double.parseDouble(tokenizer.nextToken());
                if(ratioArray[i] < -100 || ratioArray[i] > 100){
                    System.out.println("out of range for ratio!!!!");
                    return;
                }
            }            
            
            for(double ratio:ratioArray){
                inputMoney += (ratio / 100) * inputMoney;
            }
            
            
            System.out.println("earn money="+(inputMoney-10000));
            
            String format = String.format("%.0f", inputMoney-10000);
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if(button.getText().equals(TEST_BUTTON_TEXT)){
...
//                calulateStock();
                ...
                SimpleCodeTester tester = new SimpleCodeTester();
                tester.countClock();
...
//                List<ChordInfo> chordInfoList = compositionFactory.getOwnMelody();
//                
//                ChordPlayer chordPlayer = new ChordPlayer(chordInfoList, mMidiSynth);
//                chordPlayer.init();
//                chordPlayer.play(true, false);                
...
//                Groove.start();
            ...
            } else if (button.getText().startsWith("All")) {
...
//                for(ChordInfo chordInfo: chordInfoList){
//                    chordInfo.print();
//                }
//                
//                for(int i=24; i<36; i++){
//                    int key = CompositionUtil.convertKey(i);
